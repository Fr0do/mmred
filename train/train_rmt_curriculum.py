...
from train.modeling_rmt.language_modeling_qwen import MemoryCell, RecurrentWrapper
from trl import (
    TrlParser,
    ModelConfig,
    SFTConfig,
...
def preprocess_function(example, ds_args: DatasetArgs, tokenizer, segment_size: int):
    ...
    tokenized_prompt = tokenizer.apply_chat_template(
        prompt, add_generation_prompt=True, thinking_mode=False, tokenize=True, return_dict=True
    )
    ...
    user_end_idx = (
        ...
        - 1
        - next(i for i, ids in enumerate(tokenized_prompt.input_ids[::-1]) if ids == tokenizer.eos_token_id)
    )
