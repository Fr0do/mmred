        ...
        for seq_len in SEQ_LENGTHS:
            ...
            for question_type, question_fn in QUESTIONS.items():
                ...
                for _ in range(N_QUESTIONS):
                    ...
                    seq, q, a, atype, h = _generate_question(question_fn, seq_len, q_hashes, **q_kwargs)
                    q_hashes.append(h)
                    q_dataset.append({'qid': f'{q_id:07d}', 'seq_len': seq_len, 'qtype': question_type, 'atype': atype,
                                      'question': q, 'answer': a,
                                      'sequence': f'sequences/seq_{q_id:07d}.csv', 'video': f'videos/vid_{q_id:07d}'})
...
def generate_videos(base_path, exp_name):
    ...
    for seq_len in SEQ_LENGTHS:
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            executor.map(partial(_dataset_entry_to_video, dataset_path=dataset_path), q_dataset)
        et = time.time()
        print(f'Finished videos generation for len_{seq_len}. Elapsed time: {et - st:.1f} s')
