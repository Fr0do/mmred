...
from qgen.const import CHARS, NOBODY, ROOMS, AnswerTypePerson, AnswerTypeRoom, AnswerTypeNumber
from qgen.utils import (inv1d_argmax, generate_sequence_df, sample_comparison, sample_steps,
                        get_random_situation, get_random_mmlong)
...
def q_first_app(seq_len, inv=False, **kwargs):
    ...
    q = f'In which room was {char} at the final step?' if inv else f'In which room did {char} first appear?'
...
def q_char_on_char_first_app(seq_len, inv=False, **kwargs):
    ...
    q = f'In which room was {char_0} when {char_1} made their final appearance in the {room}?' \
        if inv else f'In which room was {char_0} when {char_1} first appeared in the {room}?'
    return df, q, a, AnswerTypeRoom
...
def q_first_at_room(seq_len, inv=False, **kwargs):
    ...
    q = f'Who was the last to appear in the {room}?' if inv else f'Who was the first to appear in the {room}?'
...
def q_room_on_char_first_app(seq_len, inv=False, **kwargs):
    ...
    q = f'Who was in the {room_0} when {char} made their final appearance in the {room_1}?' \
        if inv else f'Who was in the {room_0} when {char} first appeared in the {room_1}?'
    return df, q, a, AnswerTypePerson
...
def q_n_room_on_char_first_app(seq_len, inv=False, **kwargs):
    ...
    def _check_df_return_answer(_df):
        apps = (_df[char] == room_1).values
        return np.sum(_df.iloc[argmax_fn(apps)].values == room_0).item() if apps.any() else None
    ...
    q = f'How many characters were in the {room_0} when {char} made their final appearance in the {room_1}?' \
        if inv else f'How many characters were in the {room_0} when {char} first appeared in the {room_1}?'
...
def q_room_empty(seq_len, fraction: float = 1, is_more: bool = None, **kwargs):
    ...
    df = generate_sequence_df(seq_len)
    is_more, q_start, frame_0, frame_1, q_end = get_random_mmlong(seq_len, fraction, superlative=False, is_more=is_more)
...
def q_where_spend(seq_len, fraction: float = 1, is_more: bool = None, **kwargs):
    ...
    df, char, _, _, _, _ = get_random_situation(seq_len)
    is_more, q_start, frame_0, frame_1, q_end = get_random_mmlong(seq_len, fraction, is_more=is_more)
    ...
    def _check_df_return_answer(_df):
        ...
        visits = pd.concat((visits, pd.Series(data=[0] * len(unvisited), index=unvisited, dtype=np.int64)))
        if (visits.iloc[0] == visits.iloc[1]) if is_more else (visits.iloc[-1] == visits.iloc[-2]):
            return
...
def q_crowd_count(seq_len, fraction: float = 1, n_crowd: int = 3, **kwargs):
    ...
    df = generate_sequence_df(seq_len)
    _, _, frame_0, frame_1, q_end = get_random_mmlong(seq_len, fraction)
    a = df.iloc[frame_0:frame_1 + 1].apply(
        lambda x: np.sum(np.unique(x, return_counts=True)[1] >= n_crowd), axis=1).sum().item()
